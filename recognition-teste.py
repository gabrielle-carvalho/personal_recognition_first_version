...
class PeopleRecognition(Node):
    ...
    def recognition_callback(self, msg):
        ...
        try:
            # Converte a mensagem recebida em uma imagem OpenCV
        ...
        for face in faces:
            ...
            if name != "Unknown":
                ...
                if self.detection_count[name] == 8:
                    # Publica o nome da pessoa reconhecida
                    ...
                    # Reinicializa o contador após a publicação
                    self.detection_count[name] = 0
            else:
    ...
    #   VER SE FUNCIONA SEM O RECOGNITION COMPLETED SO COM O NOME:
...
class App:
    def __init__(self, master, people_recognition_node):
        ...
        self.open_camera()  # Tenta abrir a câmera
        self.update_frame()

    def open_camera(self):
        max_attempts = 50000  # Número máximo de tentativas
        attempts = 0
        
        while attempts < max_attempts:
            self.cap = cv2.VideoCapture(0)  # Use the correct camera index
            if self.cap.isOpened():
                self.people_recognition_node.get_logger().info("Camera opened successfully.")
                return
            else:
                self.people_recognition_node.get_logger().warning("Failed to open camera. Retrying...")
                attempts += 1
                cv2.waitKey(1000) #1 segundo
        
        self.people_recognition_node.get_logger().error("Unable to open camera after multiple attempts.")

    def update_frame(self):
        if self.cap is None or not self.cap.isOpened():
            self.people_recognition_node.get_logger().error("Camera is not available.")
            self.open_camera()  # Tenta abrir a câmera novamente
            return
